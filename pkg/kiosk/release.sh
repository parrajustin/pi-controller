...
IMAGE_NAME="xerofuzzion/lounge_kiosks"

echo "Building and pushing version ${TAGNAME}-x86_64 and latest-x86_64..."
docker buildx build \
  --platform linux/amd64 -t "${IMAGE_NAME}:${TAGNAME}-x86_64" -t "${IMAGE_NAME}:latest-x86_64" --output type=image,push=true,compression=zstd,force-compression=true,compression-level=3 .

echo "Building and pushing version ${TAGNAME}-aarch64 and latest-aarch64..."
docker buildx build \
  --platform linux/arm64 -t "${IMAGE_NAME}:${TAGNAME}-aarch64" -t "${IMAGE_NAME}:latest-aarch64" --output type=image,push=true,compression=zstd,force-compression=true,compression-level=3 .
